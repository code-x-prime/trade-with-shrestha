...
export default function EditWebinarPage() {
  ...
  const [removeFlags, setRemoveFlags] = useState({ image: false, instructorImage: false });
  ...
  const fetchWebinar = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Convert date to local timezone for datetime-local input
        ...
        setFormData({
          title: webinar.title || '',
          type: webinar.type || 'LIVE',
          description: webinar.description || '',
          instructorName: webinar.instructorName || '',
          instructorDescription: webinar.instructorDescription || '',
          instructorYearsExperience: webinar.instructorYearsExperience?.toString() || '',
          price: webinar.price?.toString() || '0',
          salePrice: webinar.salePrice?.toString() || '',
          isFree: webinar.isFree || false,
          seatType: webinar.seatType || 'UNLIMITED',
          maxSeats: webinar.maxSeats?.toString() || '',
          startDate: dateStr,
          duration: webinar.duration?.toString() || '',
          numberOfSessions: webinar.numberOfSessions?.toString() || '',
          language: webinar.language || '',
          accessDuration: webinar.accessDuration || '',
          liveDoubtSolving: webinar.liveDoubtSolving || false,
          accessToRecordings: webinar.accessToRecordings || false,
          recordingsAccessDuration: webinar.recordingsAccessDuration || '',
          thumbnailVideoUrl: webinar.thumbnailVideoUrl || '',
          googleMeetLink: webinar.googleMeetLink || '',
          isPublished: webinar.isPublished || false,
        });

        if (webinar.imageUrl) setImagePreview(webinar.imageUrl);
        if (webinar.instructorImageUrl) setInstructorImagePreview(webinar.instructorImageUrl);

        if (webinar.whatYouWillLearn && Array.isArray(webinar.whatYouWillLearn)) {
          setWhatYouWillLearn(webinar.whatYouWillLearn.length > 0 ? webinar.whatYouWillLearn : ['']);
        }

        if (webinar.faqs && Array.isArray(webinar.faqs)) {
          setFaqs(webinar.faqs.length > 0 ? webinar.faqs : [{ question: '', answer: '' }]);
        }
      }
    } catch (error) {
      toast.error(error.message || 'Failed to fetch webinar');
  ...
  const removeImage = (type) => {
    if (type === 'image') {
      ...
      setRemoveFlags({ ...removeFlags, image: true });
    } else {
      setInstructorImage(null);
      setInstructorImagePreview(null);
      setRemoveFlags({ ...removeFlags, instructorImage: true });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (imagePreview && !image && !removeFlags.image) {
      ...
      } else if (instructorImagePreview && !instructorImage && !removeFlags.instructorImage) {
        // URL from MediaPicker
        submitFormData.append('instructorImageUrl', instructorImagePreview);
      }

      // Send remove flags
      if (removeFlags.image) submitFormData.append('removeImage', 'true');
      if (removeFlags.instructorImage) submitFormData.append('removeInstructorImage', 'true');
      ...
      const response = await webinarAPI.updateWebinar(id, submitFormData);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Webinar</h1>
          <p className="text-muted-foreground mt-1">Update webinar details</p>
        </div>
      </div>
      ...
      <MediaPicker
        ...
        onSelect={(url) => {
          setImagePreview(url);
          setImage(null);
          setRemoveFlags(prev => ({ ...prev, image: false }));
        }}
        type="image"
      ...
      <MediaPicker
        ...
        onSelect={(url) => {
          ...
          setInstructorImage(null);
          setRemoveFlags(prev => ({ ...prev, instructorImage: false }));
        }}
        type="image"
