...
export default function EditMentorshipPage() {
  ...
  const [existingInstructorImage, setExistingInstructorImage] = useState(null);
    ...
    try {
      ...
      if (response.success) {
        ...
        setFormData({
          ...
          pricingType: mentorship.pricingType || 'PAID',
          price: mentorship.price?.toString() || '0',
          salePrice: mentorship.salePrice?.toString() || '',
          isFree: mentorship.isFree || false,
          status: mentorship.status || 'DRAFT',
          totalSessions: mentorship.totalSessions?.toString() || '',
          startDate: startDateStr,
          endDate: endDateStr,
          googleMeetLink: mentorship.googleMeetLink || '',
          instructorName: mentorship.instructorName || '',
          instructorBio: mentorship.instructorBio || '',
        });

        if (mentorship.coverImageUrl) {
          setExistingCoverImage(mentorship.coverImageUrl);
        }
        if (mentorship.instructorImageUrl) {
          setExistingInstructorImage(mentorship.instructorImageUrl);
        }

        setProgramOverview(Array.isArray(mentorship.programOverview) ? mentorship.programOverview : ['']);
        setWhoIsThisFor(Array.isArray(mentorship.whoIsThisFor) ? mentorship.whoIsThisFor : ['']);
        setWhatYouWillLearn(Array.isArray(mentorship.whatYouWillLearn) ? mentorship.whatYouWillLearn : ['']);
        setKeyConceptsRequired(Array.isArray(mentorship.keyConceptsRequired) ? mentorship.keyConceptsRequired : ['']);
        setFaqs(Array.isArray(mentorship.faqs) && mentorship.faqs.length > 0 ? mentorship.faqs : [{ question: '', answer: '' }]);
      }
    } catch (error) {
  ...
  const removeImage = (type) => {
    if (type === 'coverImage') {
      ...
      setRemoveCoverImage(true); // Set flag to remove image from R2
    } else {
      setInstructorImage(null);
      setInstructorImagePreview(null);
      setExistingInstructorImage(null);
      setRemoveInstructorImage(true); // Set flag to remove image from R2
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (instructorImagePreview && !instructorImage && !removeInstructorImage) {
        // URL from MediaPicker
        submitFormData.append('instructorImageUrl', instructorImagePreview);
      }
      if (removeCoverImage) submitFormData.append('removeCoverImage', 'true');
      if (removeInstructorImage) submitFormData.append('removeInstructorImage', 'true');
      ...
      const response = await mentorshipAPI.updateMentorship(id, submitFormData);
      ...
      toast.error(error.message || 'Failed to update mentorship program');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Mentorship Program</h1>
          <p className="text-muted-foreground mt-1">Update mentorship program details</p>
        </div>
      </div>
              ...
              {(coverImagePreview || existingCoverImage) ? (
                <div className="mt-2 relative">
                  <div className="relative w-full aspect-video rounded-lg overflow-hidden border">
                    <Image src={coverImagePreview || existingCoverImage} alt="Preview" fill className="object-cover" />
                  </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-start justify-between">
                <p className="text-sm text-blue-900 flex-1">
                  <strong>Note:</strong> Manage individual sessions with specific dates and times from the sessions page.
                </p>
                <Link href={`/admin/mentorship/${id}/sessions`}>
        ...
        <Card className="mt-6">
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              {(instructorImagePreview || existingInstructorImage) ? (
                <div className="mt-2 relative">
                  <div className="relative w-32 h-32 rounded-full overflow-hidden border">
                    <Image src={instructorImagePreview || existingInstructorImage} alt="Preview" fill className="object-cover" />
                  </div>
                  <div className="mt-2 flex gap-2">
                    <Button
                      type="button"
