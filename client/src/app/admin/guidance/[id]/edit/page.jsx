...
export default function EditGuidancePage() {
  ...
  const [removeExpertImage, setRemoveExpertImage] = useState(false);
    ...
    try {
      ...
      if (response.success) {
        ...
        setFormData({
          ...
          expertBio: guidance.expertBio || '',
          language: guidance.language || '',
          price: guidance.price?.toString() || '',
          durationMinutes: guidance.durationMinutes?.toString() || '60',
          googleMeetLink: guidance.googleMeetLink || '',
          status: guidance.status || 'ACTIVE',
        });

        if (guidance.expertImageUrl) setExpertImagePreview(guidance.expertImageUrl);

        if (guidance.expertise && Array.isArray(guidance.expertise)) {
          setExpertise(guidance.expertise.length > 0 ? guidance.expertise : ['']);
        }
      }
    } catch (error) {
  ...
  const removeImage = () => {
    ...
    setRemoveExpertImage(true); // Set flag to remove image from R2
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (expertImagePreview && !expertImage && !removeExpertImage) {
        // URL from MediaPicker
        submitFormData.append('expertImageUrl', expertImagePreview);
      }
      if (removeExpertImage) submitFormData.append('removeExpertImage', 'true');
      ...
      const response = await guidanceAPI.updateGuidance(id, submitFormData);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit 1:1 Guidance</h1>
          <p className="text-muted-foreground mt-1">Update guidance details</p>
        </div>
      </div>
