...
export default function EditIndicatorPage() {
  ...
  const fetchIndicator = async () => {
    try {
      setFetching(true);
      const response = await indicatorAPI.getIndicatorById(params.id);
      if (response.success) {
        const indicator = response.data.indicator;
        setFormData({
          name: indicator.name,
          description: indicator.description,
          videoUrl: indicator.videoUrl || '',
        });
        setExistingImageUrl(indicator.imageUrl);
      }
    } catch (error) {
      console.error('Error fetching indicator:', error);
      toast.error('Failed to fetch indicator');
    } finally {
      setFetching(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update indicator details</CardDescription>
        </CardHeader>
        <CardContent>
