...
const TYPEWRITER_WORDS = [
    { text: 'Web Development', gradient: 'from-slate-600 via-blue-600 to-slate-600' },
    ...
    { text: 'Market Analysis', gradient: 'from-blue-600 via-slate-600 to-blue-600' },
    { text: 'SQL & Databases', gradient: 'from-pink-600 via-rose-600 to-pink-600' },
    { text: 'Financial Markets', gradient: 'from-indigo-600 via-slate-600 to-indigo-600' },
    { text: 'Power BI', gradient: 'from-blue-600 via-blue-600 to-blue-600' },
    { text: 'Machine Learning', gradient: 'from-cyan-600 via-blue-600 to-cyan-600' },
    { text: 'Cyber Security', gradient: 'from-blue-600 via-fuchsia-600 to-blue-600' },
];

// Trust indicators / stats
const TRUST_STATS = [
    { icon: Users, value: '25,000+', label: 'Happy Learners', color: 'text-slate-600', bgColor: 'bg-slate-50' },
    { icon: Star, value: '4.9/5', label: 'Average Rating', color: 'text-slate-500', bgColor: 'bg-slate-50' },
...
const cardThemes = [
    { bg: 'from-slate-500/10 to-blue-500/10', border: 'border-slate-200/50', icon: 'text-slate-500', badge: 'bg-slate-100 text-slate-700' },
    ...
    { bg: 'from-blue-500/10 to-slate-500/10', border: 'border-blue-200/50', icon: 'text-blue-500', badge: 'bg-blue-100 text-blue-700' },
...
const TypewriterText = () => {
    ...
    if (!mounted) {
        return <span className="font-bold bg-gradient-to-r from-slate-600 via-blue-600 to-slate-600 bg-clip-text text-transparent">Web Development</span>;
    }
    ...
    return (
        <span className="relative inline-block">
            ...
            <motion.span
                ...
                className="inline-block w-[3px] sm:w-1 h-[0.85em] bg-slate-600 ml-1 align-middle rounded-full"
            />
        </span>
    );
};
...
const TrustIndicators = () => (
    ...
    >
        {TRUST_STATS.map((stat, index) => (
            <motion.div
                key={stat.label}
                className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border border-gray-200/60 dark:border-gray-700/60 rounded-full shadow-sm hover:shadow-md hover:border-slate-200/60 dark:hover:border-slate-500/40 transition-all duration-300"
...
const HeroContent = () => (
    <div className="relative z-20 text-center max-w-5xl mx-auto px-4 sm:px-6 py-16 sm:py-20 md:py-0">
        {/* Trust Badge */}
        <motion.div
            className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 mb-6 sm:mb-8 rounded-full bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-slate-200/60 dark:border-slate-500/40 shadow-lg shadow-slate-500/5"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
        >
            <span className="flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gradient-to-r from-slate-500 to-blue-600">
                <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white" />
            </span>
            <span className="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-200">
                Trusted by <span className="font-bold text-slate-600 dark:text-slate-400">25,000+</span> learners
            </span>
        ...
        >
            <Button
                asChild
                size="lg"
                className="w-full sm:w-auto group relative bg-gradient-to-r from-slate-600 to-blue-600 hover:from-slate-700 hover:to-blue-700 text-white px-6 sm:px-8 py-6 sm:py-7 text-base font-semibold rounded-xl sm:rounded-2xl shadow-xl shadow-slate-500/25 hover:shadow-2xl hover:shadow-slate-500/30 transition-all duration-300 hover:-translate-y-0.5"
            >
            ...
            <Button
                ...
                variant="outline"
                size="lg"
                className="w-full sm:w-auto group bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm hover:bg-slate-50/80 dark:hover:bg-slate-950/50 text-gray-700 dark:text-gray-200 hover:text-slate-700 dark:hover:text-slate-300 border-2 border-gray-200 dark:border-gray-700 hover:border-slate-300 dark:hover:border-slate-500 px-6 sm:px-8 py-6 sm:py-7 text-base font-semibold rounded-xl sm:rounded-2xl transition-all duration-300"
            >
        ...
        >
            ...
            {POPULAR_TOPICS.map((topic, index) => (
                <motion.span
                    key={topic}
                    className="px-2.5 sm:px-3 py-1 sm:py-1.5 text-xs font-medium text-gray-600 bg-gray-100/80 hover:bg-slate-100 hover:text-slate-700 rounded-full cursor-pointer transition-colors duration-200"
                    initial={{ opacity: 0, scale: 0.9 }}
                    animate={{ opacity: 1, scale: 1 }}
